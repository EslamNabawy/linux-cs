  ...
  categories: [
    {
      ...
      icon: "folder",
      commands: [
        { command: "pwd", description: "Print working directory", example: "pwd", notes: "Useful in scripts to get absolute paths — combine with $(pwd).  Tip: in scripts prefer $(pwd) — `echo \"Running in $(pwd)\"` — and `pwd -P` resolves symlinks vs `pwd -L`." },
        { command: "ls", description: "List directory contents", example: "ls -la", notes: "-a shows dotfiles (.bashrc etc.); -h with -l gives human-readable sizes.  Try `ls -lhtr` (human, time, reverse) and `ls -li` to see inodes. For spaces: `ls -b` escapes them." },
        { command: "cd", description: "Change directory", example: "cd /var/log", notes: "cd - jumps back to previous directory; cd alone goes home.  `cd -` is a stack of 1 — use `pushd`/`popd` or `cd $OLDPWD` for more. `cd` with no arg is `cd $HOME`." },
        { command: "tree", description: "List contents in a tree-like format", example: "tree -L 2", notes: "Not installed by default — sudo apt install tree.  If missing: `sudo apt install tree` or fallback `find . -maxdepth 2 -type d | sort` (faster, no color)." }
      ]
    },
    {
      id: "manipulation",
      ...
      icon: "file",
      commands: [
        { command: "touch", description: "Create an empty file / update timestamp", example: "touch newfile.txt", notes: "If the file exists, it only updates the modified timestamp — no data is lost.  For spaces: `touch \"my file.txt\"` — and `touch -- -dashfile` creates a file starting with dash. `touch -t 202501010101 file` sets timestamp." },
        { command: "mkdir", description: "Make a directory", example: "mkdir -p dir/subdir", notes: "Without -p, it fails if parent directories don't already exist.  Quote parents with spaces: `mkdir -p \"My Projects/src\"` — without -p fails if parent missing." },
        { command: "cp", description: "Copy files/directories", example: "cp -r source_dir/ dest_dir/", notes: "-r is required for directories; use -i to get a prompt before overwriting.  Use `cp -a` to preserve perms/symlinks (archive). For safety: `cp -i` prompts, `cp -n` no-clobber. Never `cp -r /etc/* /` without verify." },
        { command: "mv", description: "Move or rename files", example: "mv oldname.txt newname.txt", notes: "No -i prompt by default — silently overwrites an existing destination file.  Add `-i` to prompt before overwrite. Across filesystems `mv` copies+deletes — use `rsync --remove-source-files` for large moves." },
        { command: "rm", description: "Remove files/directories", example: "rm -rf folder/", notes: "There is no Recycle Bin. rm -rf is permanent and irreversible — double-check the path.  Danger: `rm -rf /tmp/*` OK, but `rm -rf / tmp/*` (space) = disaster. Use `rm -- -file` for dash files. Check with `ls` first, or `trash-put`." }
      ]
    },
    {
      id: "viewing",
      ...
      icon: "eye",
      commands: [
        { command: "cat", description: "Concatenate and print file content", example: "cat file.txt", notes: "Not ideal for huge files — it dumps everything at once; use less instead.  For numbers: `cat -n file` or better `nl -ba`. For large: `less` or `bat` (syntax). `cat file1 file2 > combined` is its true concatenate use." },
        { command: "less", description: "View file content page by page", example: "less largefile.log", notes: "/searchterm searches forward, n repeats the search — much better than more.  Inside less: `g` top, `G` bottom, `/error` + `n` next, `N` prev, `F` follow (like tail -F). Quieter than `more`." },
        { command: "head/tail", description: "View first/last lines of a file", example: "tail -f app.log", notes: "-f is the go-to for watching live logs; Ctrl+C to stop following.  `tail -n +5 file` from line 5 onward. `head -c 100` first bytes. For live: `tail -F` follows across rotations (better than -f)." },
        { command: "grep", description: "Search text within files", example: 'grep -i "error" /var/log/syslog', notes: "-r for recursive search, -n for line numbers, -v to invert (exclude matches).  Speed: `grep -F \"literal\"` (fixed) 10× faster than regex. Limit scope: `grep -R --include=\"*.conf\"`. Use `grep -n` for line numbers." },
        { command: "nano/vim", description: "Terminal text editors", example: "nano config.yaml", notes: "nano is beginner-friendly; vim has a steeper curve — :q! quits without saving.  nano: `Ctrl+O` write, `Ctrl+X` exit. vim: `vim -R` read-only, `:%s/old/new/gc` with confirm. For quick edit, `nano` is safest." }
      ]
    },
    {
      id: "system",
      ...
      icon: "cpu",
      commands: [
        { command: "top/htop", description: "Interactive process viewer", example: "htop", notes: "htop is friendlier and color-coded but usually needs installing separately.  Press `1` in top/htop to see per-CPU. `htop` → `F6` sort, `k` kill, `u` filter user. For batch: `ps aux --sort=-%cpu | head`." },
        { command: "ps", description: "Snapshot of current processes", example: "ps aux | grep python", notes: "aux shows all processes for all users, not just yours — combine with grep to filter.  `ps -ef --forest` or `ps auxf` shows tree. Combine: `ps aux --sort=-%mem | head -n 10` for memory hogs." },
        { command: "kill", description: "Terminate a process by PID", example: "kill -9 1234", notes: "-9 force-kills without letting the process clean up — try plain kill (SIGTERM) first.  Try `kill -TERM 1234` (15) first, then `kill -KILL` (9) only if needed — -9 skips cleanup. Find PID via `pgrep -a python`." },
        { command: "systemctl", description: "Manage system services", example: "sudo systemctl restart nginx", notes: "Use systemctl status service_name to check if something actually started correctly.  Check first: `systemctl status nginx` — `enabled` vs `active`. `systemctl is-enabled` vs `is-active`. Logs via `journalctl -u nginx -f`." },
        { command: "df", description: "Disk space usage", example: "df -h", notes: "Checks whole filesystems/partitions — good first check when you get 'no space left on device'.  Inodes matter too: `df -i` — “No space left” can be inode exhaustion, not bytes. Bind mounts skew `df` — use `findmnt`." },
        { command: "du", description: "Directory space usage", example: "du -sh /var/log", notes: "Without -s it recursively lists every subfolder's size — can be slow on large trees.  `du -sh --max-depth=1 /var` limits depth (fast). `du -sh * | sort -rh` finds biggest. For interactive: `ncdu`." }
      ]
    },
    {
      id: "networking",
      ...
      icon: "network",
      commands: [
        { command: "chmod", description: "Change file permissions", example: "chmod +x script.sh", notes: "Never use chmod 777 casually — it grants everyone full read/write/execute access.  Numeric vs symbolic: `chmod 644 file` (= rw-r--r--) vs `chmod u+x,go-w`. Check `umask 022` — default perms. Never 777 on web roots — use 755/644." },
        { command: "chown", description: "Change file owner", example: "sudo chown user:group file.txt", notes: "Usually requires sudo; changing ownership on system files can break services.  Recursive danger: `chown -R user:group /` breaks system. Always verify with `ls -ld` first. For groups: `chgrp` is clearer." },
        { command: "ip", description: "Show/manipulate IP addresses", example: "ip a", notes: "Replaces the older, deprecated ifconfig on most modern distros.  Modern: `ip -br a` brief, `ip route` default gw, `ss -tulpn` replaces `netstat`. `ifconfig` is deprecated — use `ip`." },
        { command: "curl", description: "Transfer data from URLs", example: "curl -O https://example.com/file.zip", notes: "-O saves with the remote filename; add -L to follow redirects.  Add `-L` to follow redirects (`curl -L -O https://.../file.zip`). For headers: `-I`. Never `curl http://x | bash` without `-sSf` + verify." },
        { command: "ssh", description: "Secure shell remote login", example: "ssh username@192.168.1.10", notes: "Add -p PORT for a non-default port; key-based auth (ssh-keygen) is safer than passwords.  Generate ed25519: `ssh-keygen -t ed25519 -C \"host\"` — safer than RSA. Disable root: `PermitRootLogin no` in sshd_config." }
      ]
    },
    {
      id: "archiving",
      ...
      icon: "folder",
      commands: [
        { command: "tar", description: "Archive files into a tarball", example: "tar -czvf archive.tar.gz folder/", notes: "-c create, -z gzip, -v verbose, -f file. Use -x to extract: tar -xzvf archive.tar.gz.  Preserve perms: `tar -p -czf`. Danger absolute: `tar -czf backup.tar.gz -P /etc` stores leading / — restores can overwrite. Omit -P to make relative." },
        { command: "gzip/gunzip", description: "Compress or decompress files", example: "gzip file.log", notes: "gzip replaces the file with file.log.gz; use -k to keep the original, gunzip to reverse.  Keep original: `gzip -k file.log` (keep) vs default replaces. Test: `gzip -t file.gz` verifies integrity." },
        { command: "zip/unzip", description: "Package files into a .zip archive", example: "zip -r app.zip app/", notes: "Cross-platform archives; -r is required to include subdirectories.  For permission preserve on Linux, prefer `tar.gz`. `zip -r9` best compress, `-e` encrypt (weak)." },
        { command: "rsync", description: "Efficiently sync files/directories", example: "rsync -avz src/ user@host:/dest", notes: "-a preserves permissions, -v verbose, -z compresses. Great for backups and deploys.  Dry-run first: `rsync -avzn src/ dst/` (-n). Show progress: `--progress` or `--info=progress2`. For large: `--partial` resume." }
      ]
    },
    {
      id: "git",
      ...
      icon: "file",
      commands: [
        { command: "git clone", description: "Copy a remote repository locally", example: "git clone https://github.com/user/repo.git", notes: "Add a folder name at the end to clone into a specific directory.  Shallow: `git clone --depth 1` for huge repos. Use `--branch main` to clone single branch." },
        { command: "git status", description: "Show working tree state", example: "git status -s", notes: "-s gives a short, concise output of changed files.  Short: `git status -sb` (branch + short). For ignore check: `git check-ignore -v file`." },
        { command: "git commit", description: "Record staged changes", example: "git commit -m \"fix: typo\"", notes: "Use git add first to stage; -a commits already-tracked modified files.  Amend last: `git commit --amend`. For co-authors: add `Co-authored-by:`. Use `commit.template` for consistency." },
        { command: "git push/pull", description: "Upload or download commits", example: "git push origin main", notes: "pull = fetch + merge. Use --set-upstream (-u) the first time you push a branch.  Safer pull: `git pull --rebase` avoids merge commits. First push: `git push -u origin feature` sets upstream." },
        { command: "git branch", description: "List or create branches", example: "git checkout -b feature", notes: "checkout -b creates and switches; git merge feature integrates changes.  Delete merged: `git branch -d feature`. For remote: `git push origin --delete feature`." }
      ]
    },
    {
      id: "docker",
      ...
      icon: "cpu",
      commands: [
        { command: "docker run", description: "Run a container from an image", example: "docker run -d -p 8080:80 nginx", notes: "-d detached, -p host:container port mapping. Image is pulled if missing.  Never `--privileged` unless needed — breaks isolation. Clean after: `docker run --rm` auto-remove." },
        { command: "docker ps", description: "List running containers", example: "docker ps -a", notes: "-a shows all containers including stopped ones.  Format: `docker ps --format \"table {{.Names}}\t{{.Status}}\"` for scripts. Watch: `watch -n1 docker ps`." },
        { command: "docker build", description: "Build an image from a Dockerfile", example: "docker build -t myapp .", notes: "-t tags the image; the . is the build context (current dir).  Layer cache: order Dockerfile from least to most changing. Use `.dockerignore` to skip. Try `docker build --no-cache` to force." },
        { command: "docker compose", description: "Manage multi-container apps", example: "docker compose up -d", notes: "Reads docker-compose.yml; -d starts in the background.  Check config: `docker compose config`. For logs: `docker compose logs -f --tail 50`." }
      ]
    }
  ],
  commandsBank: [
...
,
    { command: "lsblk", category: "Disk Management", briefDescription: "Lists block devices and partitions in a tree view.", keywords: ["disk", "block", "partition", "list"], flags: [{flag:"-f", desc:"Show filesystem types"}, {flag:"-o", desc:"Custom columns"}], examples: [{code:"lsblk", desc:"Tree of disks/partitions"}, {code:"lsblk -f", desc:"With FS and UUID"}, {code:"lsblk -o NAME,SIZE,FSTYPE,MOUNTPOINT", desc:"Custom output"}], pitfall: "lsblk without -f hides FS type — beginners think disk is empty", related: ["blkid", "fdisk", "mount"], difficulty: "beginner" },
    { command: "blkid", category: "Disk Management", briefDescription: "Shows block device UUID, TYPE and attributes.", keywords: ["uuid", "filesystem", "blkid"], flags: [{flag:"-o", desc:"Output format list/device"}], examples: [{code:"blkid", desc:"All devices"}, {code:"blkid /dev/sda1", desc:"Single partition"}], pitfall: "Needs sudo for some devices", related: ["lsblk", "ls -l /dev/disk/by-uuid"], difficulty: "beginner" },
    { command: "fdisk", category: "Disk Management", briefDescription: "Partitions a disk (MBR/GPT).", keywords: ["partition", "fdisk", "disk"], flags: [{flag:"-l", desc:"List partitions"}, {flag:"-u", desc:"Units"}], examples: [{code:"sudo fdisk -l", desc:"List all"}, {code:"sudo fdisk /dev/sda", desc:"Interactive"}], pitfall: "Writes immediately — double-check device", related: ["parted", "lsblk"], difficulty: "intermediate" },
    { command: "parted", category: "Disk Management", briefDescription: "Advanced partition editor (supports GPT).", keywords: ["partition", "parted", "gpt"], flags: [{flag:"-l", desc:"List"}], examples: [{code:"sudo parted -l", desc:"List"}], pitfall: "parted mklabel wipes table", related: ["fdisk"], difficulty: "intermediate" },
    { command: "mkfs.ext4", category: "Disk Management", briefDescription: "Creates an ext4 filesystem.", keywords: ["format", "mkfs", "ext4"], flags: [{flag:"-L", desc:"Label"}, {flag:"-F", desc:"Force"}], examples: [{code:"sudo mkfs.ext4 -L data /dev/sdb1", desc:"Format with label"}], pitfall: "Formats — destroys data", related: ["mkfs", "fdisk", "mount"], difficulty: "intermediate" },
    { command: "mount", category: "Disk Management", briefDescription: "Mounts a filesystem.", keywords: ["mount", "filesystem"], flags: [{flag:"-t", desc:"Type"}, {flag:"-o", desc:"Options"}], examples: [{code:"mount | column -t", desc:"Show mounts"}, {code:"sudo mount /dev/sdb1 /mnt", desc:"Mount"}], pitfall: "Mount point must exist", related: ["umount", "findmnt"], difficulty: "beginner" },
    { command: "umount", category: "Disk Management", briefDescription: "Unmounts a filesystem.", keywords: ["unmount", "umount"], flags: [{flag:"-l", desc:"Lazy"}, {flag:"-f", desc:"Force"}], examples: [{code:"sudo umount /mnt", desc:"Unmount"}, {code:"sudo umount -l /mnt", desc:"Lazy"}], pitfall: "Device busy? Use lsof", related: ["mount", "lsof"], difficulty: "beginner" },
    { command: "findmnt", category: "Disk Management", briefDescription: "Finds and lists mounted filesystems.", keywords: ["findmnt", "mount"], flags: [{flag:"-T", desc:"Target file"}], examples: [{code:"findmnt", desc:"Tree"}, {code:"findmnt -T /home", desc:"For path"}], related: ["mount", "df"], difficulty: "beginner" },
    { command: "stat", category: "File System", briefDescription: "Shows detailed file status (inode, times).", keywords: ["stat", "inode", "metadata"], flags: [{flag:"-c", desc:"Format"}, {flag:"-f", desc:"Filesystem"}], examples: [{code:"stat file.txt", desc:"File info"}, {code:"stat -c '%a %n' file", desc:"Perms"}], pitfall: "stat vs ls -l: stat shows all three times", related: ["ls", "ls -i"], difficulty: "beginner" },
    { command: "file", category: "File System", briefDescription: "Determines file type by content.", keywords: ["file", "type", "magic"], flags: [{flag:"-b", desc:"Brief"}, {flag:"-i", desc:"MIME"}], examples: [{code:"file /bin/ls", desc:"ELF binary"}, {code:"file -i image.png", desc:"MIME"}], related: ["stat", "ls"], difficulty: "beginner" },
    { command: "ncdu", category: "Disk Management", briefDescription: "Interactive disk usage analyzer (ncurses).", keywords: ["ncdu", "disk", "usage"], flags: [], examples: [{code:"ncdu /", desc:"Scan root"}, {code:"ncdu -x /home", desc:"Stay on FS"}], pitfall: "Needs install: sudo apt install ncdu", related: ["du", "df"], difficulty: "beginner" },
    { command: "free", category: "System Info", briefDescription: "Already in bank but enriched — displays RAM/swap.", keywords: ["free", "memory", "ram"], flags: [{flag:"-h", desc:"Human"}, {flag:"-m", desc:"MB"}], examples: [{code:"free -h", desc:"Human readable"}], pitfall: "free vs available — use available column", related: ["vmstat", "top"], difficulty: "beginner" },
    { command: "vmstat", category: "System Monitoring", briefDescription: "Reports virtual memory, processes, CPU.", keywords: ["vmstat", "memory", "cpu"], flags: [{flag:"-s", desc:"Stats"}], examples: [{code:"vmstat 1 5", desc:"5 samples 1s"}], related: ["free", "iostat"], difficulty: "intermediate" },
    { command: "iostat", category: "System Monitoring", briefDescription: "CPU and I/O statistics.", keywords: ["iostat", "io", "cpu"], flags: [], examples: [{code:"iostat -x 1", desc:"Extended"}], related: ["vmstat", "sar"], difficulty: "intermediate" },
    { command: "mpstat", category: "System Monitoring", briefDescription: "Per-CPU statistics.", keywords: ["mpstat", "cpu"], flags: [{flag:"-P", desc:"CPU"}], examples: [{code:"mpstat -P ALL 1", desc:"All CPUs"}], related: ["iostat"], difficulty: "intermediate" },
    { command: "dmesg", category: "System Info", briefDescription: "Kernel ring buffer messages.", keywords: ["dmesg", "kernel", "log"], flags: [{flag:"-H", desc:"Human"}, {flag:"-T", desc:"Time"}], examples: [{code:"dmesg | tail", desc:"Recent"}, {code:"dmesg --level=err", desc:"Errors"}], pitfall: "Needs sudo on some distros", related: ["journalctl"], difficulty: "beginner" },
    { command: "hostnamectl", category: "System Info", briefDescription: "Controls system hostname.", keywords: ["hostname", "hostnamectl"], flags: [], examples: [{code:"hostnamectl", desc:"Status"}, {code:"sudo hostnamectl set-hostname web01", desc:"Set"}], related: ["hostname", "uname"], difficulty: "beginner" },
    { command: "timedatectl", category: "System Info", briefDescription: "Controls time, date, timezone, NTP.", keywords: ["time", "date", "timedatectl"], flags: [], examples: [{code:"timedatectl", desc:"Status"}, {code:"sudo timedatectl set-timezone UTC", desc:"Set TZ"}], related: ["date"], difficulty: "beginner" },
    { command: "lsmod", category: "System Info", briefDescription: "Lists loaded kernel modules.", keywords: ["lsmod", "module", "kernel"], flags: [], examples: [{code:"lsmod | grep kvm", desc:"Search KVM"}], related: ["modprobe", "dmesg"], difficulty: "beginner" },
    { command: "modprobe", category: "System Info", briefDescription: "Adds/removes kernel modules.", keywords: ["modprobe", "module"], flags: [{flag:"-r", desc:"Remove"}], examples: [{code:"sudo modprobe -r kvm_intel", desc:"Remove"}], related: ["lsmod"], difficulty: "intermediate" },
    { command: "lspci", category: "System Info", briefDescription: "Lists PCI devices.", keywords: ["lspci", "pci", "hardware"], flags: [{flag:"-k", desc:"Kernel driver"}], examples: [{code:"lspci -k", desc:"With drivers"}], related: ["lsusb", "lshw"], difficulty: "beginner" },
    { command: "lsusb", category: "System Info", briefDescription: "Lists USB devices.", keywords: ["lsusb", "usb"], flags: [], examples: [{code:"lsusb", desc:"USB list"}], related: ["lspci"], difficulty: "beginner" },
    { command: "id", category: "User Management", briefDescription: "Prints user and group IDs.", keywords: ["id", "user", "group"], flags: [{flag:"-u", desc:"UID"}, {flag:"-g", desc:"GID"}], examples: [{code:"id", desc:"Current"}, {code:"id www-data", desc:"Specific user"}], related: ["whoami", "groups"], difficulty: "beginner" },
    { command: "groups", category: "User Management", briefDescription: "Shows groups for a user.", keywords: ["groups", "group"], flags: [], examples: [{code:"groups", desc:"My groups"}, {code:"groups alice", desc:"Alice"}], related: ["id", "usermod"], difficulty: "beginner" },
    { command: "who", category: "User Management", briefDescription: "Shows logged-in users.", keywords: ["who", "login"], flags: [{flag:"-a", desc:"All"}], examples: [{code:"who", desc:"Logged in"}], related: ["w", "last"], difficulty: "beginner" },
    { command: "w", category: "User Management", briefDescription: "Shows logged-in users and activity.", keywords: ["w", "who"], flags: [], examples: [{code:"w", desc:"Users + load"}], related: ["who", "uptime"], difficulty: "beginner" },
    { command: "last", category: "User Management", briefDescription: "Shows last logins from wtmp.", keywords: ["last", "login", "wtmp"], flags: [], examples: [{code:"last | head", desc:"Recent"}], related: ["who", "lastlog"], difficulty: "beginner" },
    { command: "lastlog", category: "User Management", briefDescription: "Reports most recent login per user.", keywords: ["lastlog"], flags: [], examples: [{code:"lastlog", desc:"All users"}], related: ["last"], difficulty: "beginner" },
    { command: "userdel", category: "User Management", briefDescription: "Deletes a user account.", keywords: ["userdel", "delete", "user"], flags: [{flag:"-r", desc:"Remove home"}], examples: [{code:"sudo userdel -r olduser", desc:"Delete + home"}], pitfall: "Without -r leaves home", related: ["useradd", "usermod"], difficulty: "intermediate" },
    { command: "groupdel", category: "User Management", briefDescription: "Deletes a group.", keywords: ["groupdel", "group"], flags: [], examples: [{code:"sudo groupdel devs", desc:"Delete"}], related: ["groupadd"], difficulty: "intermediate" },
    { command: "chage", category: "User Management", briefDescription: "Changes password expiry info.", keywords: ["chage", "password", "expiry"], flags: [{flag:"-l", desc:"List"}, {flag:"-E", desc:"Expire date"}], examples: [{code:"chage -l alice", desc:"List"}], related: ["passwd", "usermod"], difficulty: "intermediate" },
    { command: "chsh", category: "User Management", briefDescription: "Changes login shell.", keywords: ["chsh", "shell"], flags: [{flag:"-s", desc:"Shell"}], examples: [{code:"chsh -s /bin/zsh", desc:"To zsh"}], related: ["usermod", "echo $SHELL"], difficulty: "beginner" },
    { command: "umask", category: "Permissions", briefDescription: "Sets default creation permissions mask.", keywords: ["umask", "permission", "mask"], flags: [], examples: [{code:"umask", desc:"Show"}, {code:"umask 022", desc:"Set"}], pitfall: "022 → 755/644 ; 027 → 750/640", related: ["chmod"], difficulty: "intermediate" },
    { command: "getfacl", category: "Permissions", briefDescription: "Gets file ACLs.", keywords: ["getfacl", "acl"], flags: [], examples: [{code:"getfacl file.txt", desc:"Show ACL"}], related: ["setfacl", "chmod"], difficulty: "intermediate" },
    { command: "setfacl", category: "Permissions", briefDescription: "Sets file ACLs.", keywords: ["setfacl", "acl"], flags: [{flag:"-m", desc:"Modify"}, {flag:"-x", desc:"Remove"}], examples: [{code:"setfacl -m u:alice:rwx file", desc:"Grant alice"}], related: ["getfacl"], difficulty: "intermediate" },
    { command: "chgrp", category: "Permissions", briefDescription: "Changes group ownership.", keywords: ["chgrp", "group", "permission"], flags: [{flag:"-R", desc:"Recursive"}], examples: [{code:"chgrp devs file.txt", desc:"Change group"}], related: ["chown", "chmod"], difficulty: "beginner" },
    { command: "pgrep", category: "Process Management", briefDescription: "Finds PIDs by name.", keywords: ["pgrep", "process", "pid"], flags: [{flag:"-a", desc:"Full"}, {flag:"-u", desc:"User"}], examples: [{code:"pgrep -a python", desc:"Find python"}], related: ["ps", "pkill"], difficulty: "beginner" },
    { command: "pkill", category: "Process Management", briefDescription: "Signals processes by name.", keywords: ["pkill", "kill", "process"], flags: [{flag:"-f", desc:"Full"}], examples: [{code:"pkill -f 'python app.py'", desc:"Kill by full"}], pitfall: "pkill without -f matches only name", related: ["pgrep", "kill"], difficulty: "beginner" },
    { command: "pidof", category: "Process Management", briefDescription: "Finds PID of program.", keywords: ["pidof", "pid"], flags: [], examples: [{code:"pidof nginx", desc:"PID"}], related: ["pgrep"], difficulty: "beginner" },
    { command: "pstree", category: "Process Management", briefDescription: "Shows processes as a tree.", keywords: ["pstree", "process", "tree"], flags: [{flag:"-p", desc:"PIDs"}], examples: [{code:"pstree -p", desc:"With PIDs"}], related: ["ps", "top"], difficulty: "beginner" },
    { command: "jobs", category: "Process Management", briefDescription: "Lists active jobs.", keywords: ["jobs", "job", "background"], flags: [{flag:"-l", desc:"PID"}], examples: [{code:"jobs -l", desc:"List"}], related: ["bg", "fg"], difficulty: "beginner" },
    { command: "bg", category: "Process Management", briefDescription: "Resumes job in background.", keywords: ["bg", "background", "job"], flags: [], examples: [{code:"bg %1", desc:"Background job 1"}], related: ["fg", "jobs"], difficulty: "beginner" },
    { command: "fg", category: "Process Management", briefDescription: "Brings job to foreground.", keywords: ["fg", "foreground"], flags: [], examples: [{code:"fg %1", desc:"Foreground"}], related: ["bg", "jobs"], difficulty: "beginner" },
    { command: "nohup", category: "Process Management", briefDescription: "Runs command immune to hangup.", keywords: ["nohup", "hangup", "background"], flags: [], examples: [{code:"nohup ./long.sh &", desc:"Persist after logout"}], pitfall: "Output goes to nohup.out", related: ["tmux", "screen"], difficulty: "beginner" },
    { command: "tmux", category: "Process Management", briefDescription: "Terminal multiplexer.", keywords: ["tmux", "multiplexer", "session"], flags: [], examples: [{code:"tmux", desc:"New session"}, {code:"tmux ls", desc:"List"}], related: ["screen", "nohup"], difficulty: "intermediate" },
    { command: "screen", category: "Process Management", briefDescription: "Terminal multiplexer (legacy).", keywords: ["screen", "multiplexer"], flags: [], examples: [{code:"screen -S work", desc:"Named session"}], related: ["tmux"], difficulty: "intermediate" },
    { command: "nice", category: "Process Management", briefDescription: "Runs command with priority.", keywords: ["nice", "priority", "niceness"], flags: [{flag:"-n", desc:"Value -20..19"}], examples: [{code:"nice -n 10 ./heavy.sh", desc:"Low priority"}], related: ["renice", "top"], difficulty: "intermediate" },
    { command: "renice", category: "Process Management", briefDescription: "Alters priority of running process.", keywords: ["renice", "priority"], flags: [], examples: [{code:"renice -n 10 -p 1234", desc:"Change PID"}], related: ["nice", "ps"], difficulty: "intermediate" },
    { command: "traceroute", category: "Networking", briefDescription: "Traces route to host.", keywords: ["traceroute", "route", "hop"], flags: [], examples: [{code:"traceroute 8.8.8.8", desc:"Trace"}], related: ["mtr", "ping"], difficulty: "beginner" },
    { command: "mtr", category: "Networking", briefDescription: "Combines ping + traceroute live.", keywords: ["mtr", "ping", "trace"], flags: [], examples: [{code:"mtr 8.8.8.8", desc:"Live"}], related: ["traceroute", "ping"], difficulty: "beginner" },
    { command: "dig", category: "Networking", briefDescription: "DNS lookup utility.", keywords: ["dig", "dns", "lookup"], flags: [{flag:"+short", desc:"Short"}], examples: [{code:"dig example.com", desc:"A record"}, {code:"dig +short google.com", desc:"Short"}], related: ["host", "nslookup"], difficulty: "beginner" },
    { command: "nslookup", category: "Networking", briefDescription: "Interactive DNS query (legacy).", keywords: ["nslookup", "dns"], flags: [], examples: [{code:"nslookup example.com", desc:"Query"}], related: ["dig", "host"], difficulty: "beginner" },
    { command: "host", category: "Networking", briefDescription: "Simple DNS lookup.", keywords: ["host", "dns", "lookup"], flags: [], examples: [{code:"host example.com", desc:"Lookup"}], related: ["dig", "nslookup"], difficulty: "beginner" },
    { command: "nc", category: "Networking", briefDescription: "Netcat — reads/writes TCP/UDP.", keywords: ["nc", "netcat", "tcp", "udp"], flags: [{flag:"-l", desc:"Listen"}, {flag:"-v", desc:"Verbose"}], examples: [{code:"nc -zv example.com 80", desc:"Port check"}], pitfall: "Different nc variants (openbsd vs traditional)", related: ["nmap", "ss"], difficulty: "intermediate" },
    { command: "nmap", category: "Networking", briefDescription: "Network scanner.", keywords: ["nmap", "scan", "port"], flags: [{flag:"-p", desc:"Ports"}, {flag:"-sV", desc:"Version"}], examples: [{code:"nmap -F 192.168.1.0/24", desc:"Fast scan"}], pitfall: "Scanning without permission is illegal", related: ["nc", "ss"], difficulty: "intermediate" },
    { command: "ufw", category: "Networking", briefDescription: "Uncomplicated Firewall.", keywords: ["ufw", "firewall", "ubuntu"], flags: [], examples: [{code:"sudo ufw status", desc:"Status"}, {code:"sudo ufw allow 22", desc:"Allow SSH"}], related: ["iptables", "firewall-cmd"], difficulty: "beginner" },
    { command: "iptables", category: "Networking", briefDescription: "Netfilter firewall rules.", keywords: ["iptables", "firewall", "netfilter"], flags: [{flag:"-L", desc:"List"}, {flag:"-A", desc:"Append"}], examples: [{code:"sudo iptables -L -n", desc:"List"}], pitfall: "Rules lost on reboot unless saved", related: ["ufw", "firewall-cmd"], difficulty: "advanced" },
    { command: "firewall-cmd", category: "Networking", briefDescription: "Firewalld control.", keywords: ["firewall-cmd", "firewalld", "firewall"], flags: [{flag:"--list-all", desc:"List"}], examples: [{code:"sudo firewall-cmd --list-all", desc:"All zones"}], related: ["iptables", "ufw"], difficulty: "intermediate" },
    { command: "ssh-keygen", category: "Networking", briefDescription: "Generates SSH key pair.", keywords: ["ssh-keygen", "ssh", "key"], flags: [{flag:"-t", desc:"Type"}, {flag:"-b", desc:"Bits"}], examples: [{code:"ssh-keygen -t ed25519", desc:"Ed25519"}], pitfall: "Don't overwrite existing id_ed25519", related: ["ssh", "ssh-copy-id"], difficulty: "beginner" },
    { command: "ssh-copy-id", category: "Networking", briefDescription: "Installs SSH key on remote.", keywords: ["ssh-copy-id", "ssh", "key"], flags: [{flag:"-i", desc:"Identity"}], examples: [{code:"ssh-copy-id user@host", desc:"Install"}], related: ["ssh", "ssh-keygen"], difficulty: "beginner" },
    { command: "sftp", category: "Networking", briefDescription: "Interactive SFTP session.", keywords: ["sftp", "ftp", "secure"], flags: [], examples: [{code:"sftp user@host", desc:"Connect"}], related: ["scp", "ssh"], difficulty: "beginner" },
    { command: "bzip2", category: "Archiving", briefDescription: "Compresses with bzip2.", keywords: ["bzip2", "compress", "bzip"], flags: [{flag:"-k", desc:"Keep"}], examples: [{code:"bzip2 -k file", desc:"Keep original"}], related: ["gzip", "xz"], difficulty: "beginner" },
    { command: "xz", category: "Archiving", briefDescription: "Compresses with LZMA (high ratio).", keywords: ["xz", "compress", "lzma"], flags: [{flag:"-k", desc:"Keep"}], examples: [{code:"xz -k file", desc:"Compress"}], related: ["gzip", "bzip2"], difficulty: "beginner" },
    { command: "7z", category: "Archiving", briefDescription: "7-Zip archiver (high compression).", keywords: ["7z", "7zip", "compress"], flags: [], examples: [{code:"7z a archive.7z folder/", desc:"Create"}, {code:"7z x archive.7z", desc:"Extract"}], pitfall: "Needs p7zip-full", related: ["zip", "tar"], difficulty: "beginner" },
    { command: "dd", category: "Disk Management", briefDescription: "Copies and converts raw data.", keywords: ["dd", "copy", "raw", "disk"], flags: [{flag:"if=", desc:"Input"}, {flag:"of=", desc:"Output"}, {flag:"bs=", desc:"Block size"}], examples: [{code:"dd if=/dev/zero of=file bs=1M count=100", desc:"100M file"}, {code:"dd if=/dev/sda of=disk.img bs=4M", desc:"Clone disk"}], pitfall: "dd nickname 'disk destroyer' — triple-check of=", related: ["cp", "cat"], difficulty: "advanced" },
    { command: "tee", category: "Text Processing", briefDescription: "Splits output to file + stdout.", keywords: ["tee", "pipe", "split"], flags: [{flag:"-a", desc:"Append"}], examples: [{code:"echo hi | tee file.txt", desc:"Write + print"}, {code:"make | tee build.log", desc:"Log build"}], related: ["xargs", "tr"], difficulty: "beginner" },
    { command: "xargs", category: "Text Processing", briefDescription: "Builds and executes commands from stdin.", keywords: ["xargs", "execute", "stdin"], flags: [{flag:"-I", desc:"Replace"}, {flag:"-n", desc:"Args per"}], examples: [{code:"find . -name '*.log' | xargs rm", desc:"Delete found"}, {code:"echo a b | xargs -n1 echo", desc:"One per line"}], pitfall: "Filenames with spaces break xargs — use -0 + find -print0", related: ["find", "tee"], difficulty: "intermediate" },
    { command: "tr", category: "Text Processing", briefDescription: "Translates/deletes characters.", keywords: ["tr", "translate", "delete"], flags: [{flag:"-d", desc:"Delete"}], examples: [{code:"echo HELLO | tr 'A-Z' 'a-z'", desc:"Lowercase"}, {code:"tr -d '\r' < dos.txt", desc:"Strip CR"}], related: ["sed", "awk"], difficulty: "beginner" },
    { command: "column", category: "Text Processing", briefDescription: "Formats input into columns.", keywords: ["column", "table", "format"], flags: [{flag:"-t", desc:"Table"}], examples: [{code:"mount | column -t", desc:"Align"}], related: ["printf", "cat"], difficulty: "beginner" },
    { command: "diff", category: "Text Processing", briefDescription: "Compares files line by line.", keywords: ["diff", "compare", "patch"], flags: [{flag:"-u", desc:"Unified"}], examples: [{code:"diff -u a.txt b.txt", desc:"Unified diff"}], related: ["cmp", "patch"], difficulty: "beginner" },
    { command: "cmp", category: "Text Processing", briefDescription: "Compares files byte by byte.", keywords: ["cmp", "compare"], flags: [{flag:"-l", desc:"Verbose"}], examples: [{code:"cmp file1 file2", desc:"Compare"}], related: ["diff"], difficulty: "beginner" },
    { command: "patch", category: "Text Processing", briefDescription: "Applies diff patch to file.", keywords: ["patch", "diff", "apply"], flags: [{flag:"-p1", desc:"Strip"}], examples: [{code:"patch < fix.patch", desc:"Apply"}], related: ["diff"], difficulty: "intermediate" },
    { command: "od", category: "Text Processing", briefDescription: "Octal dump of file.", keywords: ["od", "dump", "octal"], flags: [{flag:"-c", desc:"Chars"}], examples: [{code:"od -c file", desc:"Chars"}], related: ["hexdump", "xxd"], difficulty: "intermediate" },
    { command: "hexdump", category: "Text Processing", briefDescription: "Hex dump of file.", keywords: ["hexdump", "hex", "dump"], flags: [{flag:"-C", desc:"Canonical"}], examples: [{code:"hexdump -C file", desc:"Hex+ASCII"}], related: ["od", "xxd"], difficulty: "intermediate" },
    { command: "strings", category: "Text Processing", briefDescription: "Prints printable strings in file.", keywords: ["strings", "binary", "text"], flags: [], examples: [{code:"strings /bin/ls | grep GLIBC", desc:"Find strings"}], related: ["hexdump", "file"], difficulty: "beginner" },
    { command: "env", category: "Shell & Scripting", briefDescription: "Runs program with modified environment.", keywords: ["env", "environment", "variable"], flags: [{flag:"-i", desc:"Clean"}], examples: [{code:"env | sort", desc:"List env"}, {code:"env -i bash", desc:"Clean shell"}], related: ["export", "printenv"], difficulty: "beginner" },
    { command: "export", category: "Shell & Scripting", briefDescription: "Exports variable to child processes.", keywords: ["export", "variable", "environment"], flags: [], examples: [{code:"export EDITOR=nano", desc:"Set editor"}], pitfall: "export without value just marks existing", related: ["env", "source"], difficulty: "beginner" },
    { command: "source", category: "Shell & Scripting", briefDescription: "Executes file in current shell.", keywords: ["source", "dot", ". ", "bash"], flags: [], examples: [{code:"source ~/.bashrc", desc:"Reload"}, {code:". ./vars.sh", desc:"Dot"}], related: ["bash", "export"], difficulty: "beginner" },
    { command: "type", category: "Shell & Scripting", briefDescription: "Describes how command would be interpreted.", keywords: ["type", "command", "builtin"], flags: [{flag:"-a", desc:"All"}], examples: [{code:"type ls", desc:"ls is aliased"}, {code:"type -a python", desc:"All python"}], related: ["which", "whereis"], difficulty: "beginner" },
    { command: "which", category: "Shell & Scripting", briefDescription: "Locates a command in PATH.", keywords: ["which", "path", "locate"], flags: [{flag:"-a", desc:"All"}], examples: [{code:"which python", desc:"Path"}, {code:"which -a python", desc:"All matches"}], pitfall: "which doesn't see shell functions — use type", related: ["whereis", "type"], difficulty: "beginner" },
    { command: "whereis", category: "Shell & Scripting", briefDescription: "Locates binary/source/man for command.", keywords: ["whereis", "binary", "man"], flags: [], examples: [{code:"whereis python", desc:"All paths"}], related: ["which", "type"], difficulty: "beginner" },
    { command: "whatis", category: "Shell & Scripting", briefDescription: "Displays one-line manual description.", keywords: ["whatis", "man", "description"], flags: [], examples: [{code:"whatis ls", desc:"One line"}], related: ["man", "apropos"], difficulty: "beginner" },
    { command: "man", category: "Shell & Scripting", briefDescription: "Displays manual page.", keywords: ["man", "manual", "help"], flags: [{flag:"-k", desc:"Apropos"}], examples: [{code:"man ls", desc:"Manual"}, {code:"man -k copy", desc:"Search"}], related: ["whatis", "info"], difficulty: "beginner" },
    { command: "help", category: "Shell & Scripting", briefDescription: "Shows help for shell builtin.", keywords: ["help", "builtin", "bash"], flags: [], examples: [{code:"help cd", desc:"Help for cd"}], related: ["man", "type"], difficulty: "beginner" },
    { command: "date", category: "System Info", briefDescription: "Shows or sets date/time.", keywords: ["date", "time", "clock"], flags: [{flag:"+%Y-%m-%d", desc:"Format"}], examples: [{code:"date", desc:"Now"}, {code:"date '+%F %T'", desc:"Format"}, {code:"date -d '2 days ago'", desc:"Relative"}], related: ["timedatectl", "cal"], difficulty: "beginner" },
    { command: "cal", category: "System Info", briefDescription: "Shows calendar.", keywords: ["cal", "calendar", "date"], flags: [], examples: [{code:"cal", desc:"This month"}, {code:"cal 2026", desc:"Year"}], related: ["date"], difficulty: "beginner" }
  ],
  exercises: [
    {
      id: 1,
  ...
  nti: {
    days: {
      day1: {
        ...
        },
        {
          ...
          icon: "cpu",
          blocks: [{t:"text", html:"<h6>A Brief History</h6>"}, {t:"text", html:"<p>Linux traces its lineage back to the UNIX operating system, developed through decades of incremental innovation:</p>"}, {t:"list", items:["<strong>1969</strong> — Thompson and Ritchie create AT&T UNIX.","<strong>1972</strong> — UNIX version 2 is rewritten in C.","<strong>1977</strong> — BSD (Berkeley Software Distribution) is released.","<strong>1983</strong> — Richard Stallman launches the GNU Project.","<strong>1987</strong> — Andy Tannenbaum creates Minix.","<strong>1991</strong> — Linus Torvalds creates i386 Linux.","<strong>1992</strong> — Linux is licensed under the GPL (GNU General Public License)."]}, {t:"text", html:"<p><strong>Key Takeaway:</strong> Linux emerged from decades of UNIX development, combining Torvalds' kernel with the GNU Project's tools and the GPL's open licensing model.</p>"}, {t:"text", html:"<h6>Linux Distributions</h6>"}, {t:"text", html:"<p>A <strong>Linux distribution</strong> (distro) packages the Linux kernel together with system tools, libraries, and software into a complete, installable operating system. Distributions descend from UNIX-inspired lineages and split into distinct families:</p>"}, {t:"list", items:["<strong>Debian family</strong> → Ubuntu → Linux Mint","<strong>Fedora family</strong> → RHEL → CentOS, Oracle Linux","<strong>SUSE family</strong> → SLES → OpenSUSE","Other distributions include Red Hat Linux, Fedora, and Kali Linux"]}, {t:"text", html:"<p>All distributions share the same Linux kernel at their core, but differ in package management, target use case, and support model.</p>"}, {t:"text", html:"<h6>Why Linux?</h6>"}, {t:"text", html:"<p>Linux's popularity across servers, desktops, and embedded systems comes from a combination of practical advantages:</p>"}, {t:"list", items:["<strong>Open Source</strong> — source code is freely available and modifiable.","<strong>It is Free</strong> — no licensing cost for most distributions.","<strong>High Security</strong> — a strong permissions model and active security community.","<strong>High Stability</strong> — long uptimes and reliable performance.","<strong>Ease of Maintenance</strong> — centralized package management and scripting support.","<strong>Runs on Any Hardware</strong> — from embedded devices to enterprise servers.","<strong>Customization</strong> — deep configurability at every layer.","<strong>Community Support</strong> — a large global base of contributors and users.","<strong>Education and Support</strong> — extensive documentation and learning resources."]}, {t:"callout", kind:"tip", html:"<strong>Quick check:</strong> Run <code>uname -a</code> and <code>cat /etc/os-release</code> to confirm your distro family — Debian shows <code>ID=ubuntu</code>, RHEL shows <code>ID=rhel</code>."}, {t:"code", code:"uname -r\ncat /etc/os-release | grep -E \"^(NAME|VERSION|ID)\"\nlsb_release -a 2>/dev/null || echo \"lsb_release not installed\"", lang:"bash"}]
        },
        {
          id: "linux-components",
          title: "Linux Components",
          icon: "cpu",
          blocks: [{t:"text", html:"<p>Linux systems are built from three core layers, each with a distinct role.</p>"}, {t:"text", html:"<h6>Kernel</h6>"}, {t:"text", html:"<p>The <strong>kernel</strong> is the core of the operating system.</p>"}, {t:"list", items:["Contains components such as device drivers.","Loads into RAM when the machine boots.","Stays resident in RAM until the machine powers off."]}, {t:"text", html:"<h6>Shell</h6>"}, {t:"text", html:"<p>The <strong>shell</strong> is the interface through which a user communicates with the kernel.</p>"}, {t:"list", items:["Common shells include C shell, ksh, and Bash.","<strong>Bash</strong> is the most commonly used shell on Linux.","The shell parses commands entered by the user and translates them into logical segments executed by the kernel or other utilities."]}, {t:"callout", kind:"info", html:"<strong>Key Takeaway:</strong> The kernel manages hardware and system resources, while the shell provides the interface users rely on to issue commands to that kernel."}, {t:"callout", kind:"info", html:"<strong>Mental model:</strong> CLI → Shell (bash) → Kernel → Hardware. Verify your shell with <code>echo $SHELL</code> and kernel with <code>uname -r</code>."}, {t:"code", code:"echo $SHELL\necho $BASH_VERSION\nuname -r\nps -p $$ -o comm=", lang:"bash"}]
        },
        {
          id: "vm-network",
          title: "Virtual Machines & Network Setup",
          icon: "network",
          blocks: [{t:"list", items:["<strong>Creating a VM</strong>: <code>File</code> -> <code>New VM</code> -> <code>Installing Redhat on VMware</code> (Recommended: 2 processors, 2 cores).","<strong>Network Types</strong>: Bridged, NAT, Host-only (chosen according to the IP range).","Choose <strong>NAT</strong> when setting up VMware.","<strong>Redhat Developer Account</strong>: You can use developer account credentials to log in to a Redhat server with a GUI on VMware."]}, {t:"text", html:"<h6>Getting IP & SSH</h6>"}, {t:"code", code:"ifconfig", lang:"bash"}, {t:"text", html:"<p>Returns the IP address (Skip this step for Ubuntu).</p>"}, {t:"code", code:"ssh root@ipaddress", lang:"bash"}, {t:"text", html:"<p>Connects to the server.</p>"}]
        },
        {
        ...
        },
        {
          ...
          icon: "eye",
          blocks: [{t:"text", html:"<p>There are two primary ways to reach a Linux shell:</p>"}, {t:"list", items:["<strong>From a Graphical User Interface (GUI)</strong> — opening a terminal application within a desktop environment such as GNOME.","<strong>Remote access over Secure Shell (SSH)</strong> — connecting to the system from another machine."]}, {t:"text", html:"<h6>Running Commands</h6>"}, {t:"text", html:"<p>Commands in Linux follow a consistent syntax:</p>"}, {t:"code", code:"command [options] [arguments]", lang:"bash"}, {t:"list", items:["Each item is separated by a space.","<strong>Options</strong> modify the command's behavior.","<strong>Arguments</strong> are file names or other information the command needs.","Multiple commands can be separated with a semicolon (<code>;</code>)."]}, {t:"text", html:"<h6>Example</h6>"}, {t:"code", code:"usermod –L omar", lang:"bash"}, {t:"text", html:"<h6>Command Syntax: Right vs. Wrong</h6>"}, {t:"text", html:"<p>Spacing matters. Options must be separated from the command and from each other correctly.</p>"}, {t:"table", head:["Right","Wrong"], rows:[["$ ls -l /dev","$ ls - l /dev"],["$ ls -a /dev","$ ls-a /dev"],["$ mail -s test root","$ mail test root -s"],["$ who -u","$ -u who"],["$ ls -l -d","$ ls -l-d"],["$ ls -ld","$ ls -l d"]]}, {t:"callout", kind:"info", html:"<strong>Important:</strong> Options must be attached directly to their dash (no space between <code>-</code> and the letter), and the order of arguments relative to options matters for correct parsing."}, {t:"text", html:"<h6>Shell Shortcuts</h6>"}, {t:"text", html:"<p>Bash provides keyboard shortcuts that save time when editing commands at the prompt:</p>"}, {t:"table", head:["Shortcut","Action"], rows:[["Ctrl+A","Jump to the beginning of the command line."],["Ctrl+E","Jump to the end of the command line."],["Ctrl+U","Clear from the cursor to the beginning of the command line."],["Ctrl+K","Clear from the cursor to the end of the command line."],["Ctrl+LeftArrow","Jump to the beginning of the previous word on the command line."],["Ctrl+RightArrow","Jump to the end of the next word on the command line."],["Ctrl+R","Search the history list of commands for a pattern."]]}, {t:"callout", kind:"warning", html:"<strong>Spacing trap:</strong> <code>ls - l</code> (space) ≠ <code>ls -l</code>. The shell splits on spaces — <code>-</code> must hug its letter. Also <code>mail test root -s</code> fails because <code>-s</code> must precede the address."}, {t:"code", code:"whoami; pwd; echo $HOME\nls -l /dev | head\n# try history search\n# Ctrl+R then type \"ssh\" — press Ctrl+R again to cycle", lang:"bash"}]
        },
        {
          id: "manage-files-from-the-command-line",
          title: "Manage Files from the Command Line",
          icon: "eye",
          blocks: [{t:"text", html:"<p>Linux organizes all data in a single hierarchical tree of files and directories, rooted at <code>/</code>.</p>"}, {t:"text", html:"<h6>The File System Hierarchy</h6>"}, {t:"diagram", kind:"links"}, {t:"text", html:"<p>Every directory branches from the single root (<code>/</code>), and each serves a specific, standardized purpose.</p>"}, {t:"text", html:"<h6>Important Directories</h6>"}, {t:"table", head:["Location","Purpose"], rows:[["/usr","Installed software, shared libraries, include files, and read-only program data."],["/usr/bin","User commands."],["/usr/sbin","System administration commands."],["/usr/local","Locally customized software."],["/etc","Configuration files specific to this system."],["/var","Variable data that persists between boots — databases, cache directories, log files, printer-spooled documents, and website content."],["/run","Runtime data for processes started since the last boot, including process ID and lock files. Contents are recreated on reboot; this directory consolidates /var/run and /var/lock from earlier RHEL versions."],["/home","Personal data and configuration files for regular users."],["/root","Home directory for the administrative superuser, root."],["/tmp","World-writable space for temporary files. Files untouched for 10 days are deleted automatically."],["/boot","Files needed to start the boot process."],["/dev","Special *device files* used by the system to access hardware."]]}, {t:"callout", kind:"info", html:"<strong>Note:</strong> A second temporary directory, <code>/var/tmp</code>, also exists — files there are deleted automatically only after 30 days of inactivity, giving them a longer lifespan than files in <code>/tmp</code>."}, {t:"text", html:"<h6>File Types</h6>"}, {t:"text", html:"<p>Every file in Linux has a type, indicated by a leading symbol in a long directory listing (<code>ls -l</code>):</p>"}, {t:"table", head:["Symbol","Meaning"], rows:[["-","Regular file"],["d","Directory"],["l","Link"],["c","Special File"],["s","Socket"],["p","Named Pipe"],["b","Block Device"]]}, {t:"text", html:"<h6>Rules for Naming Files</h6>"}, {t:"text", html:"<p><strong>Should:</strong><br>- Be descriptive.<br>- Use only alphanumeric characters: uppercase, lowercase, numbers, <code>@</code>, <code>_</code>.</p>"}, {t:"text", html:"<p><strong>Should Not:</strong><br>- Include embedded blanks (spaces).<br>- Contain shell metacharacters: <code>* ? > < / ; & ! [ ] | \\ ' \" ( ) { }</code></p>"}, {t:"text", html:"<p><strong>Additional rules:</strong><br>- Filenames are case sensitive.<br>- Filenames starting with a <code>.</code> are hidden.<br>- Maximum filename length is 255 characters.</p>"}, {t:"text", html:"<h6>Absolute Path vs. Relative Path</h6>"}, {t:"text", html:"<p>Every file and directory can be referenced two ways:</p>"}, {t:"list", items:["<strong>Absolute path</strong> — the full path starting from the root <code>/</code>, unambiguous regardless of the current location (e.g., <code>/home/alice/document.txt</code> or <code>/var/log/messages</code>).","<strong>Relative path</strong> — a path expressed relative to the current working directory, which changes depending on where you currently are in the file system."]}, {t:"callout", kind:"info", html:"<strong>Key Takeaway:</strong> An absolute path always starts at <code>/</code> and works from anywhere; a relative path is shorter but only valid from your current location."}, {t:"callout", kind:"danger", html:"<strong>Spaces & dashes:</strong> Quote names with spaces: <code>touch \"my notes.txt\"</code> or escape <code>touch my\\ notes.txt</code>. For leading dash: <code>touch -- -weird</code> and <code>rm -- -weird</code> — <code>--</code> ends option parsing."}, {t:"code", code:"touch \"my file.txt\" && ls -b\ntouch -- -dash && ls -l -- -dash\nrm -- -dash\nmkdir -p \"My Projects/src\" && tree -L 2 2>/dev/null || find . -maxdepth 2 | sort", lang:"bash"}]
        },
        {
          id: "inodes-hard-links-and-soft-links",
          title: "Inodes, Hard Links, and Soft Links",
          icon: "folder",
          blocks: [{t:"text", html:"<h6>What Is an Inode?</h6>"}, {t:"text", html:"<p>Linux allocates an <strong>index node (inode)</strong> for every file and directory in the filesystem. Inodes do not store the actual file data — instead, they store the metadata that points to where the file's data blocks live on disk.</p>"}, {t:"text", html:"<h6>Metadata Stored in an Inode</h6>"}, {t:"list", items:["File type","Permissions","Hard links count","Owner ID","Group ID","Soft/Hard Links","Access Control List (ACLs)","Size of file","Timestamp (access time)","Timestamp (modification time)"]}, {t:"text", html:"<p>A <strong>hard link</strong> is a direct reference to a file's inode, while a <strong>file</strong> itself is also just a name pointing to an inode. A <strong>soft link</strong> does not reference the inode directly — it points to the file name/path instead, and both hard links and soft links are maintained by the file system.</p>"}, {t:"text", html:"<h6>Soft Link vs. Hard Link</h6>"}, {t:"table", head:["Soft Link","Hard Link"], rows:[["An alias to the original file, similar to the shortcut feature in Windows OS.","The exact replica of the original file it is pointing to."],["Contains only the location to the original file, not the actual data.","Contains the actual content of the file."],["Has a different inode value pointing to the original value.","Shares the same inode value pointing to the same file location."],["Can be created across filesystems.","Cannot be created outside the filesystem."],["Becomes inaccessible when the original file is removed.","Changes in the hard-linked file will reflect in the other files."],["Can link both to a file or a directory.","Can only link to a file, not a directory."]]}, {t:"callout", kind:"info", html:"<strong>Key Takeaway:</strong> A hard link is another name for the exact same file data, while a soft link is a pointer to a file's location that breaks if the original is deleted."}, {t:"callout", kind:"tip", html:"<strong>See it live:</strong> <code>ls -i file; stat file; df -i</code> — if <code>df -i</code> shows 100% IUse, you can be “out of space” even with free GB."}, {t:"code", code:"echo hello > a.txt\nln a.txt hard.txt\nln -s a.txt soft.txt\nls -li a.txt hard.txt soft.txt\ncat soft.txt\necho world >> a.txt\ncat hard.txt # shows world too (same inode)\ncat soft.txt", lang:"bash"}]
        },
        {
          id: "pattern-matching-regex",
          title: "Pattern Matching (Regex)",
          icon: "search",
          blocks: [{t:"text", html:"<p>Bash supports pattern matching (globbing) to efficiently reference multiple files at once.</p>"}, {t:"text", html:"<h6>Shell Pattern-Matching Symbols</h6>"}, {t:"table", head:["Pattern","Matches"], rows:[["*","Any string of zero or more characters."],["?","Any single character."],["[abc...]","Any one character in the enclosed class (between the square brackets)."],["[!abc...]","Any one character *not* in the enclosed class."],["[^abc...]","Any one character *not* in the enclosed class."],["[[:alpha:]]","Any alphabetic character."],["[[:lower:]]","Any lowercase character."],["[[:upper:]]","Any uppercase character."],["[[:alnum:]]","Any alphabetic character or digit."],["[[:punct:]]","Any printable character not a space or alphanumeric."],["[[:digit:]]","Any single digit from 0 to 9."],["[[:space:]]","Any single white space character. This may include tabs, newlines, carriage returns, form feeds, or spaces."]]}, {t:"text", html:"<h6>Regular Expressions</h6>"}, {t:"text", html:"<p>Regular expressions (regex) extend pattern matching with more powerful syntax, commonly used with tools like <code>grep</code> and <code>sed</code>:</p>"}, {t:"table", head:["Symbol","Description"], rows:[[".","Replaces any character."],["^","Matches start of string."],["$","Matches end of string."],["*","Matches zero or more times the preceding character."],["\\","Represents special characters."],["()","Groups regular expressions."],["?","Matches exactly one character."]]}, {t:"callout", kind:"info", html:"<strong>Key Takeaway:</strong> Shell pattern matching (globbing) and regular expressions are related but distinct — globbing operates on filenames at the shell level, while regex is a more expressive matching language used inside text-processing tools."}, {t:"callout", kind:"info", html:"<strong>Glob vs regex:</strong> Shell glob <code>*.txt</code> is not regex <code>.*\\.txt</code>. Globs expand before the command runs — try <code>echo *.txt</code> to see."}, {t:"code", code:"touch a.txt B.TXT file1.log\nls *.txt        # glob\nls [a-z]*.txt    # range\nls [!a]*        # not a\n# regex with grep (different!)\ngrep -E \"^a.*\\.txt$\" <(ls -1)\n# speed: fixed strings\n grep -F \"error\" /var/log/syslog 2>/dev/null | head", lang:"bash"}]
        },
        {
          id: "key-takeaways",
          title: "Key Takeaways",
    ...
    notes: {
      rahma: {
        ...
        },
        {
          ...
          icon: "eye",
          blocks: [{t:"list", items:["<strong>Default Shell in Redhat</strong>: Bash","<strong>CLI (Command Line Interface)</strong> syntax: <code>Command [Option] [Argument]</code>","Dash <code>[option]</code> has \"no spaces\" (e.g., <code>-V</code>).","You must have a space after the <code>[command]</code>.","There could be no <code>[argument]</code> in CLI (used for modification/determining action)."]}, {t:"text", html:"<h6>Combining Options</h6>"}, {t:"text", html:"<p>Options can be combined.</p>"}, {t:"list", items:["Example: <code>-l -d</code> (Correct), <code>-ld</code> (Correct), <code>-l d</code> (Incorrect)."]}, {t:"text", html:"<h6>Terminal Shortcuts</h6>"}, {t:"table", head:["Shortcut","Description"], rows:[["Win + ↑","Maximize terminal"],["Ctrl + U","Delete all the part of the command before the cursor"],["Ctrl + K","Delete all the part of the command after the cursor"],["cd [tab][tab]","Displays all files with the same initial"],["cd [tab]","Auto-completes file name initial"]]}, {t:"text", html:"<h6>History Commands</h6>"}, {t:"table", head:["Command","Description"], rows:[["!500","Runs command number 500 in history"],["!!","Runs the last command in history"]]}, {t:"text", html:"<h6>Switching Users</h6>"}, {t:"code", code:"su - username", lang:"bash"}, {t:"text", html:"<p>Switches user (prompts for password).</p>"}]
        },
        {
      ...
      hager: {
        ...
        sections: [
        {
          ...
          title: "Linux Basics",
          icon: "cpu",
          blocks: [{t:"text", html:"<h6>Linux vs. Windows</h6>"}, {t:"list", items:["Security","Stability","Maintenance","Runs on different hardware","Free & open source","Easy to customize","Community support","Different user-interface experience"]}, {t:"text", html:"<h6>Linux Architecture</h6>"}, {t:"diagram", kind:"architecture"}, {t:"list", items:["<strong>Kernel:</strong> Works with the hardware.","<strong>Shell:</strong> Interface for interacting with the system through commands."]}, {t:"text", html:"<h6>Swap</h6>"}, {t:"text", html:"<p>Uses part of the storage as additional memory when needed.</p>"}]
        },
        {
    ...
    ,
    {q:"What does `ls -li` add vs `ls -l`, and why use `stat file`?", a:"`ls -li` shows inode numbers (first column) via `l`+`i`; `stat file` shows inode, size, blocks, and all three timestamps (Access/Modify/Change) plus `df -i` for inode exhaustion."},
    {q:"Why does `rm -- -f` use `--` and what does `touch \"my file\"` fix?", a:"`--` ends option parsing so `-f` is treated as filename, not flag. Spaces split arguments — quote `\"my file\"` or escape `my\\ file` to keep it one name."},
    {q:"When does `df -h` lie and `df -i` matter? Give an example.", a:"`df -h` shows bytes; `df -i` shows inodes. A disk can be “No space left” with free GB if inodes are 100% — check `df -i` and `du --maxdepth=1` vs `ncdu` for hogs."},
    {q:"What is the difference between `cp -r` and `cp -a`, and when to use `mv -i`?", a:"`cp -a` = archive (preserves perms/symlinks/xattrs) better than `-r` for backups. `mv -i` prompts before silently overwriting the destination."},
    {q:"Explain `> file 2>&1` order vs `&> file` and `PIPESTATUS`.", a:"`> file 2>&1` must have `2>&1` last — it points stderr to where stdout now goes. `&> file` is shorthand for the same. `echo ${PIPESTATUS[@]}` shows each pipe stage exit code, not just last."},
    {q:"Why does `man -k` sometimes return nothing and how to fix?", a:"`man -k` searches the `mandb` index. On fresh/minimal systems the DB may be empty — run `sudo mandb` once to build it, then `apropos`/`man -k` works."},
    {q:"In vim, what does `:%s/old/new/gc` do and how to recover from a swap file?", a:"`%` whole file, `s` substitute, `g` global, `c` confirm each. If vim warns about `.swp`, check `ls -la .*.swp` and recover with `vim -r file` then `rm .file.swp` after."},
